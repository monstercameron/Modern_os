...
const SN = { NONE: "NONE", LEFT: "LEFT", RIGHT: "RIGHT", TOP: "TOP", BOTTOM: "BOTTOM", FULL: "FULL", QUAD: "QUAD" } as const;
...
function clearBadgeState(badges: Record<string, number>, id: string) {
...
const area = (r: any) => Math.max(0, r.w) * Math.max(0, r.h);
const overlapRatio = (a: any, b: any) => {
  const inter = intersect(a, b);
  const ar = area(a);
  return ar === 0 ? 0 : area(inter) / ar;
};
const chooseBestTarget = (targets: any[], ghost: any) => {
  let bestOverlap: any = null, bestR = -1;
  let bestDist: any = null, bestD = Infinity;
...
const ghostFromPoint = (w: any, p: any) => {
...
function SnapCell({ onClick, className = "", ariaLabel = "", children }: any) {
  return (
    <button
      onClick={onClick}
...
function SnapIcon({ type }: { type: string }) {
  ...
  const rect = (x:number,y:number,w:number,h:number) => `<rect x='${x}' y='${y}' width='${w}' height='${h}' ${hi}/>`;
  let mark = "";
...
function Win({ win, on, children, active, setActive }: any) {
...
export default function DE() {
  ...
  const [wns, setW] = useState<any[]>([]);
  const [clk, setClk] = useState(tm());
  const [actId, setActId] = useState<string | null>(null);
  const [badges, setBadges] = useState<Record<string, number>>({ messages: 5, email: 3 });
  const [tests, setTests] = useState<{ ran: boolean; pass: boolean; list: any[] }>({ ran: false, pass: true, list: [] });
  const [drag, setDrag] = useState<any>({ activeId: null, targets: [], over: null, candidate: null });
  const dragRAF = useRef<number>(0);
  const dragPtRef = useRef<any>(null);
  const targetsRef = useRef<any[]>([]);
  const dragOverRef = useRef<string | null>(null);
  const [tbPrev, setTbPrev] = useState<{ id: string | null; cx: number }>({ id: null, cx: 0 });
  const tbTimer = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const b1 = clearBadgeState(b0 as any, "messages");
    ...
    try {
      ...
      const w = { b: prevB, sn: SN.LEFT, prevB, prevSN: SN.LEFT } as any;
    ...
    try {
      const w = { sn: SN.LEFT, b: { x:0,y:TB,w:100,h:100 } } as any;
  ...
  const fz = (id: string) => setW(ws => ws.map(w => ({ ...w, z: w.id === id ? 1000 : Math.max(1, w.z - 1) })));
  const setActive = (id: string) => { setActId(id); fz(id); };

  const openA = (app: any, init: any = {}) => {
    setBadges(b => clearBadgeState(b as any, app.id));
    setW(ws => {
      const id = uid();
  ...
  const act = (id: string, type: string, p?: any) => {
    setW(ws => ws.map(w => {
      if (w.id !== id) return w;
      if (type === "close") return null as any;
      ...
      if (type === "dragEnd") {
        ...
        const best = drag.candidate ? targets.find((t:any) => t.id === drag.candidate) : chooseBestTarget(targets, ghost);
  ...
  const unmin = (id: string) => act(id, "unmin");
  ...
  return (
    <div className="relative w-full h-[100vh] font-sans bg-slate-900 text-slate-900 overflow-hidden">
      ...
      <div className="absolute inset-x-0 top-12 bottom-0 p-4 grid grid-cols-6 auto-rows-[96px] gap-2">
        {APPS.map(app => (
          <Tile key={app.id} app={app} badge={(badges && (badges as any)[app.id]) || 0} onOpen={() => openA(app)} onQuick={(init: any) => openA(app, init)} />
        ))}
      </div>

      {/* Windows */}
      {wns.filter(w => !w.m).map(w => {
        const App = (APPS.find(a => a.id === w.appId)?.content || StubApp) as any;
        return (
          <Win key={w.id} win={w} active={w.id===actId} setActive={setActive} on={(t: string, p?: any) => act(w.id, t, p)}>
      ...
      {drag.activeId && drag.targets.length > 0 && (
        <div className="pointer-events-none absolute inset-x-0 top-12 bottom-0 z-[1500]">
          ...
          {drag.candidate && (() => {
            const c = drag.targets.find((x: any) => x.id === drag.candidate);
            if (!c) return null;
            return (
...
// ---------- Tiles ----------
function Tile({ app, onOpen, onQuick, badge = 0 }: any) {
  const Icon = app.icon;
  const [hv, setHv] = useState(false);
      ...
      {app.id === 'music' ? (
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={(e)=>{e.stopPropagation(); setPlaying(p=>!p);}} className="px-2 py-1 bg-black/20 border border-white/20 text-[11px]">
              {playing ? 'Pause' : 'Play'}
            </button>
          </div>
        </div>
          ...
          <motion.div animate={hv ? { x: 6, scale: 1.02 } : { x: 0, scale: 1 }} transition={{ duration: 0.12, ease: "easeOut" }}>
            <div className="font-semibold flex items-center justify-between">
              ...
              <button
                ...
                className="ml-2 px-2 py-1 bg-black/20 border border-white/20 text-[11px]"
...
// ---------- Apps ----------
function AppShell({ title, subtitle, actions, children }: any) {
  return (
